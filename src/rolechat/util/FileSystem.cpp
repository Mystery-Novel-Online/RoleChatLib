...
std::string paths::findFile(const std::string &filePath, bool allowPackages, const std::vector<std::string> &extensions)
{
    std::vector<std::string> candidates = {filePath};
    for (const auto& ext : extensions)
        candidates.push_back(filePath + ext);

    // Search in packages if allowed
    if (allowPackages) 
    {
...
std::string paths::findFile(const std::vector<std::string> &filePaths, bool allowPackages, const std::vector<std::string> &extensions)
{
